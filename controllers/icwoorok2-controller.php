...
class Icwoorok2Controller
{
    ...
    public static function init()
    {
        ...
        // Our settings are updated, cleanse cache
        add_action('update_option_icwoorok2_refresh_token', [__CLASS__, 'flushSettings']);
    ...
    public static function addPspSettings($aSettings)
    {
        ...
        $aAddedSettings[] = [
            'name' => __('Debug webhook', 'ic-woo-rabo-omnikassa-2'),
            'type' => 'checkbox',
            'label' => __('Enable the webhook debug', 'ic-woo-rabo-omnikassa-2'),
            'default' => 'no',
            'desc' => '<p>'.__('Additional logging will be activated and placed in the debug.log of Wordpress.', 'ic-woo-rabo-omnikassa-2').'</p>',
            'id' => 'icwoorok2_debugwebhook',
        ];
    ...
    public static function flushSettings()
    {
        // Refreshtoken has been updated, cleanse cache file
    ...
    public static function doNotify()
    {
        ...
        $logWebhook = get_option('icwoorok2_debugwebhook');
        $bLogWebhook = (strcmp($logWebhook, 'yes') === 0);
        ...
        if (empty($sJsonData)) {
            // Check if it is a GET request, then show a default text, otherwise error
            if ($_SERVER['REQUEST_METHOD'] === 'GET') {
                echo __('Rabo Smart Pay webhook works, however is not called correctly. This is normally done through a POST with data.', 'ic-woo-rabo-omnikassa-2');
                exit;
            }
                        ...
                        if (!empty($aResult) && sizeof($aResult)) {
                            ...
                            if (strcmp($sSignature, $sHash) === 0) {
                                foreach ($aResult['orderResults'] as $aRokResult) {
                                    ...
                                    $sMessage = __('Status received from Rabo Smart Pay Webhook: ', 'ic-woo-rabo-omnikassa-2').$sOmniKassaStatus.'. ';
                                ...
                                // Log data
                                if ($bLogWebhook) {
                                    $sLogMessage = 'Smart pay has validation failed! Signature/Signingkey incorrect';
                                    error_log($sLogMessage);
                                }
                            }
                        }
